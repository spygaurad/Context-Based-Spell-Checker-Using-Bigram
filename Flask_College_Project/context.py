        ...
        with tf.name_scope("RNN_Encoder_Cell_1D"):
            for layer in range(num_layers):
                with tf.variable_scope('encoder_{}'.format(layer)):
                    ...
                    enc_output, enc_state = tf.nn._rnn(drop,
                                                              rnn_inputs,
                                                              sequence_length,
                                                              dtype=tf.float32)
        ...
        with tf.name_scope("RNN_Encoder_Cell_2D"):
            for layer in range(num_layers):
                with tf.variable_scope('encoder_{}'.format(layer)):
                    ...
                    enc_output, enc_state = tf.nn.bidirectional__rnn(cell_fw,
                                                                            cell_bw,
                                                                            rnn_inputs,
                                                                            sequence_length,
                                                                            dtype=tf.float32)
...
def training_decoding_layer(dec_embed_input, targets_length, dec_cell, initial_state, output_layer,
    ...
    with tf.name_scope("Training_Decoder"):
        ...
        training_logits, _ = tf.contrib.seq2seq._decode(training_decoder,
...
def inference_decoding_layer(embeddings, start_token, end_token, dec_cell, initial_state, output_layer,
    ...
    with tf.name_scope("Inference_Decoder"):
        ...
        inference_logits, _ = tf.contrib.seq2seq._decode(inference_decoder,
                                                                output_time_major=False,
                                                                impute_finished=True,
                                                                maximum_iterations=max_target_length)
